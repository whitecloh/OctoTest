...
namespace OctoGames.TestTask.Gameplay.Units
{
    ...
    {
        private const string LegacyGeneratedUnitDataId = "generated_unit";
            ...
            ref int nextRuntimeId)
        {
            ...
            for (int i = 0; i < savedUnits.Count && units.Count < maxUnits; i++)
            {
                ...
                units.Add(new UnitRuntimeState(
                    runtimeId,
                    NormalizeDataId(savedUnit.DataId, fallbackDataId),
                    savedUnit.Value,
                    pointIndex,
                    pointIndex,
                    false,
        ...
        private static string NormalizeDataId(string dataId, string fallbackDataId)
        {
            if (string.IsNullOrWhiteSpace(dataId) || dataId == LegacyGeneratedUnitDataId)
            {
                return fallbackDataId;
            }
