...
{
    public sealed class UnitsStatsView : MonoBehaviour
    {
        ...
        private static string FormatStats(UnitStats stats, string format)
        {
            ...
            try
            {
                return string.Format(safeFormat, stats.TotalCount, stats.MaxCount, stats.TotalValue, stats.AverageValue);
            }
            catch (FormatException)
            {
                return string.Format("Units: {0}/{1} | Value: {2} | Avg: {3:0.##}", stats.TotalCount, stats.MaxCount, stats.TotalValue, stats.AverageValue); 
            } 
        } 
    }
}
